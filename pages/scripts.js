		...
		( json ) =>
		{
			...
			for ( u in json.uids )
			{
				...
				/* GRID */
					...
					var item = `
						<a id="` + u + `" href="https://stadiaicons.web.app/` + u + `/?fullName=` + fName.replace( '"', '%22' ).replace( ' ', '%20' ).replace( '&', '%26' ).replace( '?', '%3F' ) + `&shortName=` + sName.replace( "'", '%27' ).replace( '"', '%22' ).replace( ' ', '%20' ).replace( '&', '%26' ).replace( '?', '%3F' ) + `" target="_blank" tabindex="0">
